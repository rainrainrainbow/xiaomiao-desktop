...
static char s_last_error[ERROR_BUF_SIZE] = "";

/* 自定义打印函数：将错误输出捕获到缓冲区 */
static void error_print_strn(void *data, const char *str, size_t len)
{
    (void)data;
    size_t cur = strlen(s_last_error);
    size_t copy_len = len;
    if (cur + copy_len >= ERROR_BUF_SIZE - 1) {
        copy_len = ERROR_BUF_SIZE - 1 - cur;
    }
    if (copy_len > 0) {
        memcpy(s_last_error + cur, str, copy_len);
        s_last_error[cur + copy_len] = '\0';
    }
}

static mp_print_t s_error_printer = {
    .data = NULL,
    .print_strn = error_print_strn,
};

const char* poincare_runtime_get_last_error(void)
{
    return s_last_error;
}

void poincare_runtime_clear_error(void)
{
    s_last_error[0] = '\0';
}
...
int poincare_runtime_exec(const char *source, const char *source_name)
{
    ...
    if (lex == NULL) {
        ...
        snprintf(s_last_error, sizeof(s_last_error), "MemoryError: cannot create lexer");
        return -1;
    }
    ...
    if (nlr_push(&nlr) == 0) {
        ...
        if (mp_obj_is_exception_instance(exc)) {
            /* 打印到缓冲区（调试模式：屏幕显示） */
            mp_obj_print_exception(&s_error_printer, exc);
            /* 同时打印到串口日志 */
            mp_obj_print_exception(&mp_plat_print, exc);
        }
        ESP_LOGW(TAG, "Python exception in %s: %s", source_name ? source_name : "<string>", s_last_error);
        return -1;
    }
}
...
int poincare_runtime_exec_file(const char *filename)
{
    ...
    /* 使用文件词法分析器（不通过 pyexec_file，因为 pyexec_file 内部
     * 使用 mp_plat_print 打印异常，无法捕获到我们的缓冲区） */
    mp_lexer_t *lex = mp_lexer_new_from_file(qstr_from_str(filename));
    if (lex == NULL) {
        ESP_LOGE(TAG, "Failed to open file: %s", filename);
        snprintf(s_last_error, sizeof(s_last_error), "FileNotFoundError: %s", filename);
        return -1;
    }

    /* 异常捕获（与 poincare_runtime_exec 相同的模式） */
    nlr_buf_t nlr;
    nlr.ret_val = NULL;
    if (nlr_push(&nlr) == 0) {
        /* 编译并执行 */
        ...
        ESP_LOGW(TAG, "Python exception in %s: %s", filename, s_last_error);
        return -1;
    }
}
