...
{
    /* Fill entire screen with black pixels (required before DISPON) */
    ...
    for (uint16_t y = 0; y < LCD_V_RES; y += 8) {
        ...
        st7735_tx(io, ST7735_RAMWR, line,
...
static void st7735_init(esp_lcd_panel_io_handle_t io)
{
    ...
    /* Fill with black before turning display on */
    st7735_clear_black(io);
    /* DISPON will be sent after first LVGL flush */
}
...
static void flush_cb(lv_display_t *d, const lv_area_t *area, uint8_t *px)
{
    ...
    /* MADCTL_MV handles rotation: CASET = column = LVGL X, RASET = row = LVGL Y */
...
void app_main(void)
{
    ...
    /* Send DISPON after first LVGL flush completes (matches xiaomiao-loader approach) */
    lcd_display_on();
