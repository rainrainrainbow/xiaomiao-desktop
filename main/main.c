/*
 * Xiaomiao LCD Test Firmware - v40
 *
 * 使用最底层 spi_device 直接驱动 ST7735，跳过 esp_lcd_panel_io 抽象层。
 * 这样可以排除 esp_lcd_panel_io 的问题。
 * 同时测试不同的 SPI 时钟频率。
 */
...
static void st7735_init(void)
{
    ...
    const uint8_t caset[]    = {0x00, 0x00, 0x00, 0x7F};   /* 128 cols */
    const uint8_t raset[]    = {0x00, 0x00, 0x00, 0x9F};   /* 160 rows */
    ...
    ESP_LOGI(TAG, "ST7735 init complete (direct SPI)");
}
...
static void lcd_test_4colors(void)
{
    ...
    for (int q = 0; q < 4; q++) {
        ...
        lcd_cmd(ST7735_RAMWR);
        static uint16_t buf[(LCD_H_RES/2) * (LCD_V_RES/2)];
        for (int i = 0; i < (LCD_H_RES/2)*(LCD_V_RES/2); i++) buf[i] = colors[q];
        lcd_data((uint8_t*)buf, (LCD_H_RES/2)*(LCD_V_RES/2)*2);
    }
}
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "========== Xiaomiao LCD Test v40 (direct SPI) ==========");
