...
static volatile bool s_py_error_pending = false;  /* 脚本执行失败，需要显示错误 */
static char s_py_error_msg[128];                  /* 错误信息缓冲区 */
...
static void py_run_task(void *arg)
{
    ...
    /* 如果脚本执行失败，捕获错误信息并请求显示 */
    if (ret != 0) {
        const char *err = poincare_runtime_get_last_error();
        if (err && err[0] != '\0') {
            strncpy(s_py_error_msg, err, sizeof(s_py_error_msg) - 1);
            s_py_error_msg[sizeof(s_py_error_msg) - 1] = '\0';
        } else {
            snprintf(s_py_error_msg, sizeof(s_py_error_msg), "Script error (ret=%d)", ret);
        }
        s_py_error_pending = true;
        s_py_flush_pending = true;  /* 触发一次刷新，让错误信息上屏 */
        ESP_LOGI(TAG, "Python error captured for display: %s", s_py_error_msg);
    }
...
void app_micropython_on_tick(void)
{
    ...
    if (s_py_flush_pending) {
        ...
        /* 如果需要显示错误信息，绘制到 canvas 上 */
        if (s_py_error_pending) {
            s_py_error_pending = false;
            /* 在 canvas 上绘制错误信息（红色文字） */
            ...
            if (scr) {
                ...
                lv_label_set_text(err_lbl, buf);
                lv_obj_set_style_text_color(err_lbl, lv_color_hex(0xFF4444), 0);
                lv_obj_set_style_text_font(err_lbl, lv_font_cn_get(14), 0);
                lv_obj_set_width(err_lbl, LCD_H_RES - 8);
...
static void python_app_activate(void)
{
    ...
    if (!entry_file && strcmp(app_name, "Python") == 0) {
        ...
        if (ret == 0) {
            ...
            /* 读取调试模式下的错误详情 */
            const char *err_detail = poincare_runtime_get_last_error();
            if (err_detail && err_detail[0] != '\0') {
                /* 截断错误信息到屏幕可显示的长度 */
                static char s_err_buf[120];
                strncpy(s_err_buf, err_detail, sizeof(s_err_buf) - 1);
                s_err_buf[sizeof(s_err_buf) - 1] = '\0';
                /* 在错误信息中查找换行符，截取第一行作为屏幕显示 */
                char *nl = strchr(s_err_buf, '\n');
                if (nl) *nl = '\0';  /* 取第一行 */
                /* 如果仍然太长，截断到屏幕宽度 */
                if (strlen(s_err_buf) > 50) {
                    s_err_buf[50] = '\0';
                }
                result_msg = s_err_buf;
                ESP_LOGI(TAG, "Display error on screen: %s", s_err_buf);
            } else {
